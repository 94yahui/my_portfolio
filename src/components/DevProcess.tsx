...
const DevProcessCard = ({title, steps, color,children}:DevProcessProps)=>{
    ...
    <div className="relative flex-1 transition-all duration-500 group rounded-2xl self-stretch">
        ...
        <div className="group-hover:scale-110 group-active:scale-110 transition-all duration-500">
            {children}
        </div>
        ...
        <div className="flex gap-4 flex-row items-start">
            ...
            <p className="-m-0.75 text-[15px] dark:text-gray-400 group-hover:dark:text-gray-300 group-active:dark:text-gray-300 transition-all duration-500">{step}</p>
        </div>
        )}
        </div>
        </div>
        ...
        <div className={`absolute z-0 inset-0 ${bgDevProcessColors[color]} opacity-0 group-hover:opacity-100 group-active:opacity-100 blur-lg transition-all duration-1000 rounded-2xl`}></div>
    </div>)
}
